...
Resource that are still used in a project cannot be removed. This means disabled Texture Set still reference resources and prevent them from being deleted. To avoid that, remove disabled Texture Sets in the [ Texture Set reassignment window](../../../interface/texture-set/texture-set-reassignment.md).
...
Use the "Save and Compact" function in the [ File Menu](../../../interface/main-menu/file-menu.md) to re-save the project and remove wasted empty space. This save action will be longer than a regular save but can significantly reduce the file footprint.
