---
title: "Interface"
description: ""
helpx_description: "Painter > Interface"
---

# Interface

The interface is divided in several panels, that can be reordered, resized and even hidden.

Below is a list of the main window of the application:

* [Assets](assets/assets.md)
* [Color picker](color-picker/color-picker.md)
* [Display settings](display-settings/display-settings.md)
* [History](history/history.md)
* [Layer stack](layer-stack/layer-stack.md)
* [Main menu](main-menu/main-menu.md)
* [Project configuration](project-configuration/project-configuration.md)
* [Properties](properties/properties.md)
* [Settings](settings/settings.md)
* [Shader settings](shader-settings/shader-settings.md)
* [Texture Set](texture-set/texture-set.md)
* [Toolbars](toolbars/toolbars.md)
* [Viewport](viewport/viewport.md)

Additional pages:

* [ Shortcuts](settings/shortcuts/shortcuts.md)
* [Sliders](miscellaneous/sliders/sliders.md)
