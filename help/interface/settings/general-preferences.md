...
| **Enable GPU Raytracing** | If enabled, the Bakers will try to use the GPU for performing raytracing instead of the CPU. The feature allows bakers to perform faster in general.This can only be enabled on compatible hardware. See the [System requirements](../../getting-started/system-requirements.md) for more details. |
...
| Setting | Description |
| --- | --- |
| **Cache directory** | Defines the location where temporary files are written. This includes the [Sparse Virtual Textures](../../features/sparse-virtual-textures.md) cache. This setting can be overridden by an [Environment variables](../../pipeline-and-integration/configuration/environment-variables.md). |
